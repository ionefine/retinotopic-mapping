...
stim.rect=CenterRect([1 1 display.resolution(1) display.resolution(2)],[ 1 1 size(X)]);
...
stim.background.ringpos=linspace(.0001, max(XDeg(:))*1.2, stim.background.nrings+1);
...
if strcmp(stim.type, 'bar')
    ...
    stim.img = zeros([size(XDeg),stim.npos],'uint8');
   ...
   % stim.mf.ringpos=exp(linspace(log(stim.mf.inner), log(stim.mf.outer), stim.mf.nrings+1));% radii position of each ring
    ...
    for flipNum = 1:stim.npos % for each version of the MF sequence
      ...
      %  img(mod(T, 1)<0.05)=0;
        ...
        stim.img(:,:,flipNum) = insertImg(zeros(size(X)), img);
